= Spring Forum API

A service running at http://localhost:8080:

= Auth


== Register
.request
include::{snippets}/register/http-request.adoc[]
.response
include::{snippets}/register/http-response.adoc[]

== Login
.request
include::{snippets}/login/http-request.adoc[]
.response
include::{snippets}/login/http-response.adoc[]

== Refresh Token
.request
include::{snippets}/refreshToken/http-request.adoc[]
.response
include::{snippets}/refreshToken/http-response.adoc[]

== Change Username
.request
include::{snippets}/changeUsername/http-request.adoc[]
.response
include::{snippets}/changeUsername/http-response.adoc[]

== Change Email
.request
include::{snippets}/changeEmail/http-request.adoc[]
.response
include::{snippets}/changeEmail/http-response.adoc[]

== Change Password
.request
include::{snippets}/changePassword/http-request.adoc[]
.response
include::{snippets}/changePassword/http-response.adoc[]

== Validate New Username
.request
include::{snippets}/validateNewUsername/http-request.adoc[]
.response
include::{snippets}/validateNewUsername/http-response.adoc[]

== Validate New Email
.request
include::{snippets}/validateNewEmail/http-request.adoc[]
.response
include::{snippets}/validateNewEmail/http-response.adoc[]


= User


== Get User
.request
include::{snippets}/getUser/http-request.adoc[]
.response
include::{snippets}/getUser/http-response.adoc[]


= Group


== Create Group
.request
include::{snippets}/createGroup/http-request.adoc[]
.response
include::{snippets}/createGroup/http-response.adoc[]

== Get Groups
.request
include::{snippets}/getGroups/http-request.adoc[]
.response
include::{snippets}/getGroups/http-response.adoc[]

== Get Groups Count
.request
include::{snippets}/getGroupsCount/http-request.adoc[]
.response
include::{snippets}/getGroupsCount/http-response.adoc[]

== Get Group
.request
include::{snippets}/getGroup/http-request.adoc[]
.response
include::{snippets}/getGroup/http-response.adoc[]

== Edit Group
.request
include::{snippets}/editGroup/http-request.adoc[]
.response
include::{snippets}/editGroup/http-response.adoc[]

== Transfer Group Ownership
.request
include::{snippets}/transferOwnership/http-request.adoc[]
.response
include::{snippets}/transferOwnership/http-response.adoc[]

== Add Moderator
.request
include::{snippets}/addModerator/http-request.adoc[]
.response
include::{snippets}/addModerator/http-response.adoc[]

== Delete Moderator
.request
include::{snippets}/deleteModerator/http-request.adoc[]
.response
include::{snippets}/deleteModerator/http-response.adoc[]

== Get Subscribers
.request
include::{snippets}/getSubscribers/http-request.adoc[]
.response
include::{snippets}/getSubscribers/http-response.adoc[]

== Get Moderators
.request
include::{snippets}/getModerators/http-request.adoc[]
.response
include::{snippets}/getModerators/http-response.adoc[]

== Is Moderator
.request
include::{snippets}/isModerator/http-request.adoc[]
.response
include::{snippets}/isModerator/http-response.adoc[]

== Subscribe Group
.request
include::{snippets}/subscribeGroup/http-request.adoc[]
.response
include::{snippets}/subscribeGroup/http-response.adoc[]

== Get Subscribed Groups
.request
include::{snippets}/getSubscribedGroups/http-request.adoc[]
.response
include::{snippets}/getSubscribedGroups/http-response.adoc[]

== Validate New Group Name
.request
include::{snippets}/validateNewGroupName/http-request.adoc[]
.response
include::{snippets}/validateNewGroupName/http-response.adoc[]


= Post


== Create Post
.request
include::{snippets}/createPost/http-request.adoc[]
.response
include::{snippets}/createPost/http-response.adoc[]

== Get Posts
.request
include::{snippets}/getPosts/http-request.adoc[]
.response
include::{snippets}/getPosts/http-response.adoc[]

== Get Posts Count
.request
include::{snippets}/getPostsCount/http-request.adoc[]
.response
include::{snippets}/getPostsCount/http-response.adoc[]

== Get Post
.request
include::{snippets}/getPost/http-request.adoc[]
.response
include::{snippets}/getPost/http-response.adoc[]

== Edit Post
.request
include::{snippets}/editPost/http-request.adoc[]
.response
include::{snippets}/editPost/http-response.adoc[]

== Upvote Post
.request
include::{snippets}/upvotePostApi/http-request.adoc[]
.response
include::{snippets}/upvotePostApi/http-response.adoc[]

== Downvote Post
.request
include::{snippets}/downvotePostApi/http-request.adoc[]
.response
include::{snippets}/downvotePostApi/http-response.adoc[]

== Delete Post
.request
include::{snippets}/deletePost/http-request.adoc[]
.response
include::{snippets}/deletePost/http-response.adoc[]


= Comment

== Create Comment
.request
include::{snippets}/createComment/http-request.adoc[]
.response
include::{snippets}/createComment/http-response.adoc[]

== Get Comments
.request
include::{snippets}/getComments/http-request.adoc[]
.response
include::{snippets}/getComments/http-response.adoc[]

== Edit Comment
.request
include::{snippets}/editComment/http-request.adoc[]
.response
include::{snippets}/editComment/http-response.adoc[]

== Upvote Comment
.request
include::{snippets}/upvoteCommentApi/http-request.adoc[]
.response
include::{snippets}/upvoteCommentApi/http-response.adoc[]

== Downvote Comment
.request
include::{snippets}/downvoteCommentApi/http-request.adoc[]
.response
include::{snippets}/downvoteCommentApi/http-response.adoc[]

== Delete Comment
.request
include::{snippets}/deleteComment/http-request.adoc[]
.response
include::{snippets}/deleteComment/http-response.adoc[]